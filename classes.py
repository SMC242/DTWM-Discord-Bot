...
class commandListener():
    ...
    def listening(self):
        ...
        #tuple of lists: [command names], [functions], [arguments]
        commands=[
            "die", #command to die fast. Must be first for extra SPEEEEEEEEED
            "close", #shuts down the bot nicely
            "reset" #restarts the bot
            ], #if no args, set to None
        [os._exit, close],
        [0, self] 
        ...
        while True:
            ...
            for i, command in enumerate(commands[0]):
                if listenerInput == command:
                    if commands[2][i] is not None:
                        commands[1][i](commands[2][i])
                    else:
                        commands[1][i]()
